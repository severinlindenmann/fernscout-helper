...
/** Every optional capability a journal may opt into — lib/config.ts. */
export const FEATURES = [
  "reactions", "costs", "push", "mail", "whatsapp", "auth", "signup",
  "contacts", "postcards", "photobook", "logging", "credits", "addressLookup",
];
...
export const MODEL = {
  "config.json": {
    ...
    keys: {
      ...
      visibility: { type: "string", enum: ["public", "guest"], tip: "whether this instance advertises the journal at all. Absent reads as public" },
      ...
      features: { type: "object", tip: `what this journal switches on — ${FEATURES.join(", ")}` },
      travellers: { type: "array", tip: "how the walking figures are drawn when a trip does not say" },
  ...
  "trip.md": {
    ...
    keys: {
      ...
      status: { type: "string", enum: ["upcoming", "current", "past"], tip: "which trip the journal opens on" },
      accent: { type: "string", enum: ["sky", "yellow", "green", "coral", "navy"], tip: "the trip's colour" },
      visibility: { type: "string", enum: ["private", "public", "guest"], tip: "who may read it. private = only the people who were there; guest = everyone let into the journal. An unreadable value reads as private, never public" },
      listed: { type: "boolean", tip: "false keeps a public trip out of the sitemap, the feed and the switcher. It only ever narrows" },
      costsVisibility: { type: "string", enum: ["public", "guests"], tip: "who may see what it cost, once they can read the trip at all" },
      test: { type: "boolean", tip: "content nobody lived, written to prove the pipeline works. It says so in a banner" },
  ...
  "entries/YYYY-MM-DD-slug.md": {
    ...
    keys: {
      ...
      lng: { type: "number" },
      transportMode: { type: "string", enum: ["flight", "train", "bus", "motorbike", "boat", "car", "walk"], tip: "how you arrived; draws the leg from the previous day" },
      transportFrom: { type: "string" },
      transportTo: { type: "string" },
      travelScene: { type: "string", enum: ["default", "quick", "skip"], tip: "how the travel scene into this day plays" },
...
/** The cost line's own shape — checked wherever a `costs:` list appears. */
export const COST_KEYS = {
  label: { type: "string", required: true },
  amount: { type: "number", required: true },
  currency: { type: "string", pattern: /^[A-Za-z]{3}$/, expected: "a three-letter code like EUR" },
  category: { type: "string", enum: ["preparation", "flights", "accommodation", "food", "transport", "activities", "other"] },
};
...
/** Image and video files the instance will take — lib/validate/media.ts. */
export const MEDIA_FORMATS = { image: ["jpg", "jpeg", "png", "webp", "avif", "heic", "heif"], video: ["mp4", "mov", "webm"] };
